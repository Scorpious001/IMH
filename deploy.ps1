...
if systemctl is-active --quiet imh-ims 2>/dev/null; then \
  sudo systemctl restart imh-ims && \
  sleep 3 && \
  sudo systemctl status imh-ims --no-pager -l; \
else \
  echo "Gunicorn service not set up yet. Skipping restart." && \
  echo "Run first-time-server-setup.sh to complete server setup."; \
fi && \
