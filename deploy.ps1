...
$deployCommand = "cd ~/$REPO_PATH && git pull origin $BRANCH && cd backend && if [ ! -d 'venv' ]; then echo 'Setting up virtual environment for first time...' && if ! command -v python3-venv >/dev/null 2>&1 && ! python3 -m venv --help >/dev/null 2>&1; then echo 'Installing python3-venv package...' && sudo DEBIAN_FRONTEND=noninteractive apt-get update -qq && sudo DEBIAN_FRONTEND=noninteractive apt-get install -y python3-venv; fi && python3 -m venv venv && [ -f venv/bin/activate ] && source venv/bin/activate && pip install --upgrade pip && pip install -r requirements.txt && pip install gunicorn psycopg2-binary dj-database-url || (echo 'Virtual environment setup failed' && exit 1); else [ -f venv/bin/activate ] && source venv/bin/activate || (echo 'Virtual environment not found' && exit 1); fi && pip install -r requirements.txt --quiet && python manage.py migrate --noinput && python manage.py collectstatic --noinput && if systemctl is-active --quiet imh-ims 2>/dev/null; then sudo systemctl restart imh-ims && sleep 3 && sudo systemctl status imh-ims --no-pager -l; else echo 'Gunicorn service not set up yet. Skipping restart.' && echo 'Run first-time-server-setup.sh to complete server setup.'; fi && echo 'Backend deployment complete'"
